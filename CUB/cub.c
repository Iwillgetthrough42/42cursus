...
/*   Updated: 2021/04/12 14:49:10 by arastepa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cond(t_all *all)
{
	...
	{
		all->ray.stepx = 1;
		all->ray.sidedistx = (all->pl.mapx + 1.0 - all->pl.x) * \
		all->ray.deldistx;
	}
	if (all->ray.raydiry < 0)
	{
		all->ray.stepy = -1;
		all->ray.sidedisty = (all->pl.y - all->pl.mapy) *\
		all->ray.deldisty;
	}
	else
	{
		all->ray.stepy = 1;
		all->ray.sidedisty = (all->pl.mapy + 1.0 - all->pl.y) *\
		all->ray.deldisty;
	}
}
...
void	initvar(t_all *all, int x)
{
	...
	all->ray.deldistx = fabs(1 / all->ray.raydirx);
	all->ray.deldisty = fabs(1 / all->ray.raydiry);
	all->ray.hit = 0;
}
...
void	logic(t_all *all)
{
	...
	mlx_put_image_to_window(all->data.mlx, \
		all->data.mlx_win, all->img.img, 0, 0);
}
...
void	start(char *file, t_all *all)
{
	...
	create_data(all);
	all->data.mlx_win = mlx_new_window(all->data.mlx, all->data.resx, \
		all->data.resy, "JUMANJI");
